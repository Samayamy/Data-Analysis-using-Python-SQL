...
--find top 5 highest sellig products in each region
with cte as(
...
--find month over month growth comaprison for 2022 and 2023 sales eg: jan 2022 vs jan 2023
...
with cte as(select category,FORMAT(order_date,'MMM yyyy') as order_year_month, sum(sale_price) as sales from df_orders
group by category,FORMAT(order_date,'MMM yyyy')
--order by category,FORMAT(order_date,'MMM yyyy')
)
select * from (
